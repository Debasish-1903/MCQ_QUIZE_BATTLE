...
const QuizTopicCard = ({ topic, onStartQuiz }) => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const filteredQuiz = data.games.find(
          (game) => game.gameId === topic._id
        );
        if (filteredQuiz) {
  ...
  const handleViewScore = async () => {
    try {
      const response = await axios.get(
        `http://localhost:5000/game/${topic._id}`
      );
      const filteredGames = response.data.filter(
        (game) => game._id === `${topic._id}`
      );
        ...
        <Card.Body>
          <div className="d-flex justify-content-between align-items-center">
            <div>
              ...
              <Card.Title>
                <strong className="quiz-headline"> {topic.gameName}</strong>
              </Card.Title>
              <div>
                <strong className="quiz-paragraph">Difficulty:</strong>{" "}
                <strong className="quiz-data">{topic.difficulty}</strong>
                <br />
                <strong className="quiz-paragraph">Subject: </strong>{" "}
                <strong className="quiz-data"> {topic.subject}</strong>
                <br />
                <strong className="quiz-paragraph">
                  Number of Questions:
                </strong>{" "}
                <strong className="quiz-data">{topic.noOfQuestions} </strong>
              </div>
            </div>
            <div className="d-flex flex-column align-items-end">
              ...
              <Button
                variant="primary"
                onClick={attempted ? handleViewScore : onStartQuiz}
                className="quiz-data"
              >
