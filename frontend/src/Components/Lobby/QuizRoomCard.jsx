...
const QuizRoomCard = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const roomResponseAgain = await axios.get(
          `${API_URL}/roomFriendGame/${roomId}`
        );

        let gameIndex = roomResponseAgain.data.randomGame;
        ...
        socket.emit("game-started", {
          roomId,
          questions: games[gameIndex].mcqs,
        });
      } catch (error) {
  ...
  const handleFinish = async () => {
    ...
    const totalTimeTaken = updatedTimeTaken.reduce(
      (acc, time) => acc + time,
      0
    );
    const playerData = {
    ...
    try {
      const response = await axios.put(
        `http://localhost:5000/roomFriendGame/${roomId}`,
        {
          Finished: 1,
          removed: 0,
          playerIndex,
    ...
    const roomResponseFinished = await axios.get(
      `${API_URL}/roomFriendGame/${roomId}`
    );
    ...
    if (FinishedResult === 2) {
      ...
      if (
        ParticipatedData[0].correctAnswers >
          ParticipatedData[1].correctAnswers ||
        (ParticipatedData[0].correctAnswers ===
          ParticipatedData[1].correctAnswers &&
          ParticipatedData[0].totalTimeTaken <
            ParticipatedData[1].totalTimeTaken)
      ) {
        setWinner(player1.username);
        try {
          await axios.put(
            `http://localhost:5000/accuracyRoom/incrementCounts/${player1Email}`,
            {
              type: "win",
              subject: subject,
          ...
          await axios.put(
            `http://localhost:5000/accuracyRoom/incrementCounts/${player2Email}`,
            {
              type: "loss",
            }
          );
  ...
  const handleClose = async () => {
    ...
    const roomResponseRemoved = await axios.get(
      `${API_URL}/roomFriendGame/${roomId}`
    );
  ...
  if (!questions.length) return <div>No questions to load</div>;
    ...
    <Container className="quiz-container my-5" style={containerStyle}>
      {!showResults ? (
        <>
          <Row className="mb-3">
            <Col>
              <strong>Player 1: {player1.username}</strong>
            </Col>
            <Col>
              <strong>Player 2: {player2.username}</strong>
            </Col>
          </Row>
          <Row className="mb-3">
          ...
          <Row className="mb-3">
            <Col className="text-center">
              ...
              <Button variant="danger" onClick={handleFinish} className="ml-2">
                Finish
              </Button>
            </Col>
          </Row>
