...
io.on("connection", (socket) => {
  ...
  socket.on("join-room", async (data) => {
    ...
    console.log(
      `join-room received with roomId: ${roomId}, username: ${username}`
    );
  ...
  socket.on(
    "player-finished",
    async ({ roomId, player, email, correctAnswers, totalTimeTaken }) => {
      try {
        ...
        await room.save();
        console.log(room);
        const players = room.participants.map((p) => p.username);
        // const players = [];
...
// const roomRoutes = require("./routes/RoomRoutes");
// const accuracyRoutes = require("./routes/AccuracyRoutes");
// const roomFriendGameRoutes = require("./routes/roomFriendGameRoutes");
...
// app.use("/rooms", roomRoutes);
// app.use("/accuracy", accuracyRoutes);
// app.use("/roomFriendGame", roomFriendGameRoutes);
...
//   socket.on(
//     "player-finished",
//     async ({ roomId, player, correctAnswers, totalTimeTaken }) => {
//       try {
//         const room = await RoomFriendGame.findOne({ roomId });
//         if (!room) {
//           throw new Error("Room not found");
...
//         const opponent = room.players.find((p) => p.username !== player);
//         if (opponent && room.results[opponent.username]) {
//           io.to(roomId).emit("game-over", room.results);
//         } else {
//           io.to(roomId).emit("player-finished", {
//             player,
